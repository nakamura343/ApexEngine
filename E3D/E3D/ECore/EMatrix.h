...
namespace E3D {
	...
	struct EMatrix44 
	{
		...
		//获取矩阵的某个元素值
		inline EFloat at(size_t x, size_t y) const
		{
			return m[x][y];
		}
		...
		//矩阵的乘法
		inline EMatrix44 operator *(EMatrix44& right)const
		{
			EMatrix44 temp;
			for (EInt i = 0; i < 4; i++)
			{
		...
		{
			if (m[0][0] != right.m[0][0] || m[0][1] != right.m[0][1] || m[0][2] != right.m[0][2] || m[0][3] != right.m[0][3])return false;
			if (m[1][0] != right.m[1][0] || m[1][1] != right.m[1][1] || m[1][2] != right.m[1][2] || m[1][3] != right.m[1][3])return false;
			if (m[2][0] != right.m[2][0] || m[2][1] != right.m[2][1] || m[2][2] != right.m[2][2] || m[2][3] != right.m[2][3])return false;
			if (m[3][0] != right.m[3][0] || m[3][1] != right.m[3][1] || m[3][2] != right.m[3][2] || m[3][3] != right.m[3][3])return false;
			return true;
		}
